...
function loadAccounts() {
  if (!fs.existsSync("account.txt")) {
    console.error(
      "account.txt not found. Please add the file with account data."
    );
    process.exit(1);
  }
...
function displayHeader() {
  ...
  console.log("");
  const instructions =
    "Use 'A' to switch to the previous account, 'D' to switch to the next account, 'C' to exit.";
...
function displayAccountData(index) {
  ...
  if (messages[index].startsWith("Error:")) {
    ...
    console.log(
      `Account ${index + 1}: Potential Points: ${
        potentialPoints[index]
...
async function connectWebSocket(index) {
  ...
  sockets[index].onmessage = async (event) => {
    ...
    if (data.message === "Pulse from server") {
      console.log(
        `Pulse from server received for Account ${index + 1}. Start pinging...`
      );
      setTimeout(() => {
        startPinging(index);
      }, 10000);
...
async function updateCountdownAndPoints(index) {
  ...
  if (lastUpdateds[index]) {
    ...
    if (diff > 0) {
      ...
      countdowns[index] =
        "Calculating, it might take a minute before starting...";
